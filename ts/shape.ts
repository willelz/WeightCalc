...
export namespace Shapes {
  //直方体
  export const cuboid: Shape = {
    edges: ["vertical", "horizontal", "height"],
    calc: (density: number) => {
      const vertical = getMeter("vertical");
      const horizontal = getMeter("horizontal");
      const height = getMeter("height");
      return vertical * horizontal * height * density;
    }
  };

  //円柱
  export const cylinder: Shape = {
    edges: ["diameter", "height"],
    calc: (density: number) => {
      const r = getMeter("diameter") / 2;
      const height = getMeter("height");
      return r * r * Math.PI * height * density;
    }
  };
}
